...
export default function AppFooter() {
  ...
  return (
    <footer className="mt-10 border-t border-black/6 bg-white py-10">
      <div className="mx-auto w-full max-w-[1760px] px-4 sm:px-6 lg:px-10">
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-[1.3fr_repeat(3,minmax(0,1fr))]">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-3 text-sm">
              {workspaceLinks.map((to) => (
                <Link key={to} to={to} className="air-inline-link transition-colors hover:text-[var(--palette-bg-primary-core)]">
                  {t(`nav.${to.replace('/', '')}`)}
                </Link>
              ))}
            </div>
