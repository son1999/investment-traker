          ...
          return (
            <div key={item.symbol} className="flex flex-col gap-1.5">
              <div className="flex items-center justify-between">
                <span className="font-['JetBrains_Mono'] text-xs font-bold text-body">{item.symbol}</span>
                <span className="font-['JetBrains_Mono'] text-xs font-bold" style={{ color }}>{item.positive ? '+' : ''}{item.profitPercent.toFixed(1)}%</span>
              </div>
              <div className={`flex h-1.5 overflow-hidden rounded-full bg-field ${!item.positive ? 'flex-row-reverse' : ''}`}>
