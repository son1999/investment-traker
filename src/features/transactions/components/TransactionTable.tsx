...
export default function TransactionTable() {
  ...
  const { filter, search, page, limit, setPage, setLimit, selectedIds, toggleSelect, selectAll, deselectAll } = useTransactionsUIStore()
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-panel-alt hover:bg-panel-alt">
              ...
              {!isGuest && <TableHead className="text-center">Actions</TableHead>}
            </TableRow>
          </TableHeader>
          <TableBody>
            {transactions.map((tx) => {
              ...
              return (
                <TableRow key={tx.id} data-state={isSelected ? 'selected' : undefined}>
                  ...
                  <TableCell className="px-6 text-right font-['JetBrains_Mono'] text-sm text-body">{formatQuantity(tx.quantity)}</TableCell>
                  <TableCell className="px-6 text-right font-['JetBrains_Mono'] text-sm text-body">{formatCurrency(tx.unitPrice, tx.currency)}</TableCell>
                  <TableCell className="px-6 text-right font-['JetBrains_Mono'] text-sm font-bold text-heading">{formatCurrency(tx.quantity * tx.unitPrice, tx.currency)}</TableCell>
        ...
        <div className="flex items-center justify-between border-t border-edge-subtle bg-panel-alt px-6 py-4">
          <div className="flex items-center gap-4">
            <span className="text-sm text-caption">
              {t('transactions.showing')} <span className="font-medium text-heading">{meta && meta.total > 0 ? `${(page - 1) * limit + 1}-${Math.min(page * limit, meta.total)}` : '0'}</span>
              {t('transactions.of')} <span className="font-medium text-heading">{meta?.total || 0}</span> giao dịch
            </span>
            <div className="flex items-center gap-2">
              <span className="text-sm text-caption">{t('transactions.rowsPerPage', 'Hiển thị')}</span>
